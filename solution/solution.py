...
def run_assistant(
    ...
    while True:
        ...
        stream = retry_with_backoff(_call_api, max_retries=3, base_delay=0.1)
...
def format_comparison_table(results: list[dict]) -> str:
    ...
    for r in results:
        p = r["prompt"].replace('\n', ' ')[:37] + ("..." if len(r["prompt"]) > 37 else "")
        g_ans = r["gpt4o_answer"].replace('\n', ' ')[:37] + ("..." if len(r["gpt4o_answer"]) > 37 else "")
        m_ans = r["mini_answer"].replace('\n', ' ')[:37] + ("..." if len(r["mini_answer"]) > 37 else "")
        g_lat = f"{r['gpt4o_time']:.2f}s"
        m_lat = f"{r['mini_time']:.2f}s"
        lines.append(f"{p:<40} | {g_ans:<40} | {m_ans:<40} | {g_lat:<15} | {m_lat:<15}")
    return "\n".join(lines)
